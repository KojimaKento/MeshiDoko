...
- **コミットメッセージは日本語で記述**
- **コミットメッセージはシンプルに（1行、50文字以内推奨）**
  - ✅ 良い例: `Export履歴整理スクリプトを追加`
  - ✅ 良い例: `検索機能のバグを修正`
  - ✅ 良い例: `ユーザー認証機能を実装`
  - ❌ 悪い例: 複数の変更内容を詰め込んだ長いメッセージ
- 詳細な説明が必要な場合のみ、空行を挟んで2行目以降に追記
- 必ず`Co-Authored-By: Claude Sonnet 4.5 <noreply@anthropic.com>`を末尾に追加
